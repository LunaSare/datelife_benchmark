...
library(datelife)
aves.spp <- make_datelife_query(input="Aves", sppfromtaxon=TRUE)
# there are 12750 bird spp names
save("aves.spp", file="data/1_datasource/1_name_samples/aves.spp.RData")
load("/Users/luna/datelife_benchmark/data/1_datasource/1_name_samples/aves.spp.RData")
length(aves.spp$cleaned.names)
aves <- get_datelife_result(input = aves.spp$cleaned.names)
```
...
names(aves)[which.max(sapply(aves, nrow))]
```
Now summarize results of the `datelife` search as a `phylo` object:
```{r}
start_time <- Sys.time() # to register run time
aves_phylo <- summarize_datelife_result(datelife_query = aves.spp$cleaned.names, datelife_result = aves, summary_format = "phylo_all")
...
mat <- vector(mode = "list", length = length(tip_number))
length(mat)
for(i in seq(tip_number)){
    print(i)
    taxa <- sample(seq(nrow(aves[[27]])), tip_number[i])
    mat[[i]] <- aves[[27]][taxa, taxa]
}
sapply(mat, nrow)
trees <- summarize_datelife_result(datelife_result = mat, summary_format = "phylo_all")
plot(tree)
```
