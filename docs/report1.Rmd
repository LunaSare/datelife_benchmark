...
```{r first_test, echo = F, results = 'asis', fig.cap= ""}
figcap <-  "First try with `microbenchmark` function."
plot1_first_test <- paste0("\n![", figcap, "](plots/plot1_first_test.pdf)\n")
cat(plot1_first_test)
```
...
```{r res28, echo = F, results = 'asis', fig.cap= ""}
figcap <- "First profiling results. Profiling was run simultaneously for input size > 1 000 names."
plot2_res28 <- paste0("\n![", figcap, "](plots/plot2_res28.pdf)\n")
cat(plot2_res28)
```
...
```{r res29, echo = F, results = 'asis', fig.cap= ""}
figcap <-  "Second profiling results. Tests run consecutively on the same machine."
plotcat <- paste0("\n![", figcap, "](plots/plot3_res29.pdf)\n")
cat(plotcat)
```

This is good, but now we want a prettier plot, so we're writing some code for that next:
```{r eval = FALSE, message = FALSE}
autoplot.gfr <- function (object, log = TRUE, y_min = 0.1, y_max = 1.05 * max(object$time), 
                          flip = TRUE, xlab_angle = 90, ylab_angle = 315) {
...
```{r res29_pretty1, echo = F, results = 'asis', fig.cap= ""}
figcap <-  "Second profiling results. Tests run consecutively on the same machine. Pretty plot."
plotcat <- paste0("\n![", figcap, "](plots/plot4_res29.pdf)\n")
cat(plotcat)
```
...
```{r res29_pretty1notlog, echo = F, results = 'asis', fig.cap= ""}
figcap <-  "Second profiling results. Tests run consecutively on the same machine. Pretty plot not log."
plotcat <- paste0("\n![", figcap, "](plots/plot4_res29notlog.pdf)\n")
cat(plotcat)
# this is for next r chunk:
loadd(res29f)
res29 <- res29f
```
...
Also, now we wanna have the lebgth of input on the x axis and time on the y axis as dependent variable. So let's fix that:
...
```{r res29_pretty2, echo = F, results = 'asis', fig.cap= ""}
figcap <-  "Second profiling results. Tests run consecutively on the same machine. Pretty plot with time in y axis and good x axis labels."
plotcat <- paste0("\n![", figcap, "](plots/plot5_res29.pdf)\n")
cat(plotcat)
```


It's messy to have to change the names of the profile object in order to change the labels of the x axis, so let's make a function to try different types of x and y axis labeling without modifying the microbenchmark profiling object:

```{r eval = FALSE}
xaxis_label <- function(object, xaxis_type = 1){
  if(xaxis_type == 1){
    plt <- object + ggplot2::scale_x_discrete(name = "Query Length (Number of names, log10)", 
                   labels=c("10 names" = "1", 
                            ...
                            "all names (12750)" = expression(1.275*"x"*10^4) 
                   ))
  }
  if(xaxis_type == 2){
    plt <- object + ggplot2::scale_x_discrete(name = "Query Length (Number of names)", 
                   labels=c("10 names" = "10", 
                            ...
                            "all names (12750)" = expression(12~750) 
                   ))
  }
...
yaxis_label <- function(object, yaxis_type = 1){
  if(yaxis_type == 1){
    plt <- 	plt + ggplot2::scale_y_log10(name="Time (seconds)", 
               breaks=c(1e+03, 1e+04, 3e+04, 1e+05), 
               labels=c("1e+03"="1 s", "1e+04"="10 s", "3e+04"="30 s", 
                        "1e+05"="100 s"), 
               position="left",
               sec.axis = ggplot2::sec_axis(~ . *1, name="Time (minutes)", 
                  breaks=c(6e+03, 3e+04, 6e+04, 9e+04), 
                  labels=c("6e+03"="0.1 min", "3e+04"="0.5 min", 
                           "6e+04"="1 min", "9e+04"="1.5 min")))
  }
}
```
